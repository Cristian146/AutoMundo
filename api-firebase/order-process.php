...
if(isset($_POST['update_order_status']) && isset($_POST['id'])) {
    ...
    if(!empty($res)){
    	...
    	foreach($status as $each){
    		if (in_array($postStatus, $each)) {
    			$response['error'] = true;
    			$response['message'] = isset($_POST['delivery_boy_id']) && $_POST['delivery_boy_id'] != '' && ($res[0]['delivery_boy_id']!=0)?'Vendedor actualizado, pedido ya '.$postStatus:'Pedido ya '.$postStatus;
    			print_r(json_encode($response));
    			return false;
    		}
    	}
    ...
    // 		echo $country_code;
        ...
        // 		echo $sql;
    		...
    		$response['error'] = true;
    		$response['message'] = isset($_POST['delivery_boy_id']) && $_POST['delivery_boy_id'] != ''?'vendedor actualizado, pero no se pudo actualizar el estado del pedido, intente nuevamente!':'No se pudo actualizar el estado del pedido, intente nuevamente!';
    		print_r(json_encode($response));
    	}
    }else{
		$response['error'] = true;
