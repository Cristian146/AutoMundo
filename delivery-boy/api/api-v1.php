    ...
    */
	...
	if($num == 1){
	    if($res[0]['status'] == 0){
            ...
            $response['message'] = "Vendedor fue conectado con éxito";
            $response['data'] = $res;
		}
	}else{
...
if(isset($_POST['get_delivery_boy_by_id'])){
    ...
    */
	...
	if($num == 1){
    	$response['error'] = false;
        $response['message'] = "Datos del vendedo obtenidos con éxito";
        $response['data'] = $res;
	}else{
		$response['error'] = true;
...
if(isset($_POST['update_delivery_boy_profile'])){
    ...
    if(!isset($_POST['id']) || empty(trim($_POST['id'])) || !is_numeric($_POST['id'])){
        ...
        $json_response['message'] = "Identificación no válida del vendedor";
        print_r(json_encode($json_response));
        return false;
        exit();
    }
    ...
    if($num != 1){
        $json_response['error'] = true;
        $json_response['message'] = "Vendedor no está registrado.";
        print_r(json_encode($json_response));
        return false;
        exit();
    }
    ...
    if(!empty($res)){
    	...
    	foreach($status as $each){
    		if (in_array($postStatus, $each)) {
    			$response['error'] = true;
    			$response['message'] = 'Vendedor actualizado, pero el estado del pedido no se debe a un estado duplicado!';
    			print_r(json_encode($response));
    			return false;
    		}
    	}
    	...
    	if($postStatus=='entregado'){
    		...
    		if($res_boy[0]['delivery_boy_id']!=0){
    			...
    			$db->sql($sql);
    			$comission=$fn->add_delivery_boy_commission($delivery_boy_id,'credit',$reward,$message='Comisión de vendedor de pedidos.');
        ...
        $final_status = array(
     	...
     	if($db->update('orders',$final_status,'id='.$id)){
    		...
    		$response['error'] = true;
    		$response['message'] = "Vendedor actualizado, pero no se pudo actualizar el estado del pedido. ¡Inténtalo de nuevo!";
    		print_r(json_encode($response));
    	}
    }else{
		$response['error'] = true;
...
if(isset($_POST['update_delivery_boy_fcm_id'])){
    ...
    if(isset($_POST['fcm_id']) && !empty($_POST['fcm_id'])){
	    ...
	    if($db->sql($sql1)){
	        $response['error'] = false;
            $response['message'] = "Vendedor fcm_id Actualizado con éxito.";
            print_r(json_encode($response));
	    } else {
	        $response['error'] = true;
            $response['message'] = "No puedo actualizar fcm_id of vendedor.";
            print_r(json_encode($response));
	    }
   }
}
